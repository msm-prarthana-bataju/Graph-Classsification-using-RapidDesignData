...
#     tagset = list(tagset)
#     tag2index = {tagset[i]: i for i in range(min(len(tagset), 71))}  # Limit node feature size to 71

#     for g in g_list:
#         g.node_features = np.zeros((len(g.node_tags), 71), dtype=np.float32)  # Fixed node feature size to 71
#         for i, tag in enumerate(g.node_tags):
#             if tag in tag2index:
#                 index = tag2index[tag]
#                 if index < 71:
#                     g.node_features[i, index] = 1


#     print('# classes: %d' % len(label_dict))
#     print('# maximum node tag: %d' % min(len(tagset), 71))  # Print the limited node feature size
...
# def my_load_data(dataset, degree_as_tag=False, batch_size=64, num_workers=4):
#     dataset_path = dataset
#     device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

#     custom_dataset = MyDataset(dataset_path, degree_as_tag=degree_as_tag)
#     dataloader = DataLoader(custom_dataset, batch_size=batch_size, shuffle=True, num_workers=num_workers)

#     label_dict = custom_dataset.label_dict
#     tagset = set([])
#     for g in custom_dataset.g_list:
#         tagset = tagset.union(set(g.node_tags))
...
#     print('# classes: %d' % len(label_dict))
#     print('# maximum node tag: %d' % len(tagset))
#     print("# data: %d" % len(custom_dataset))

#     return dataloader, len(label_dict)
