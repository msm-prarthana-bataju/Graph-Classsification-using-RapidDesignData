...
parser.add_argument("--dataset", default="D:/3DStepGraphClassification_RapidDesignData/Datasets/GraphData_05/", help="Name of the graph (.graphml) dataset.")
...
model_name =  "GCN_model_" + str(datetime.today().strftime('%m-%d'))
# out_dir = os.path.abspath(os.path.join(args.run_folder, "./results/runs_GCN", args.dataset))
out_dir = os.path.abspath(os.path.join(args.run_folder, "./results/runs_GCN", model_name))
...
# mem.start_monitoring("memory_usage_log_2.txt")
# Load Graph data
# ==================================================
print("Loading data...")
use_degree_as_tag = False
fold = 0
graphs, num_classes = my_load_data_without_graph_load(args.dataset)
...
for epoch in range(1, args.num_epochs + 1):
    ...
    # if not isinstance(valid_graphs, S2VGraph):
    valid_graphs = load_validation_graph(valid_graphs, device=device)
