...
class GCN_CN_v3(nn.Module):
    ...
    # def forward(self, adj, features):
    # # Assuming convolution_1, convolution_2, and convolution_3 are instances of nn.Module and can handle edge_index
    #     features = F.relu(self.convolution_1(x=features, edge_index=adj))
    #     features = F.dropout(features, p=self.dropout, training=self.training)
        
    #     features = F.relu(self.convolution_2(x=features, edge_index=adj))
    #     features = F.dropout(features, p=self.dropout, training=self.training)
        
    #     features = F.relu(self.convolution_3(x=features, edge_index=adj))
    #     features = F.dropout(features, p=self.dropout, training=self.training)

    #     pooled_features = self.attention(features)
    #     pooled_features = torch.t(pooled_features)

    #     scores = F.relu(self.fully_connected_first(pooled_features))
    #     scores = self.scoring_layer(scores)
    #     score = F.log_softmax(scores, dim=1)
    #     return score
